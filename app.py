...
def get_data_dict():
    ...
    try:
        ...
        # --- 3-Way Tiebreakers (Strictly: Toronto + 1 AL East rival + 1 non-East rival OR 1 Central + 1 West) ---
        east_tracked = [t for t in target_teams if t in AL_EAST]
        other_tracked = [t for t in target_teams if t not in AL_EAST]
        
        valid_3way_pairs = set()
        
        # Combination Type A: Toronto + 1 AL East rival + 1 non-East rival (Central/West)
        for e_team in east_tracked:
            for o_team in other_tracked:
                pair = tuple(sorted([e_team, o_team]))
                valid_3way_pairs.add(pair)
                
        # Combination Type B: Toronto + 1 Central rival + 1 West rival
        ...
        for c_team in cen_tracked:
            for w_team in wes_tracked:
                pair = tuple(sorted([c_team, w_team]))
                valid_3way_pairs.add(pair)

        for team1, team2 in valid_3way_pairs:
